    ...
    effectListData.forEach(async (ef) => {
        ...
        if (ef.modSpecKey.startsWith("macro") && doMacros) {
            ...
            if (ef.modSpecKey === "macro.execute" || ef.modSpecKey === "macro.macroExecute") { // maintain compat for mistaken version
                ...
                if (tokenMagic && token) {
                    requestGMAction(GMAction.actions.applyTokenMagic, { userId: game.user.id, tokenId: token.id, effectId: fields[0] });
                    let duration = validateDuration(itemData.data.duration);
                    // if about time installed
                    if (aboutTimeInstalled) {
                        let spec = {};
                        if (duration.value !== 0)
                            ...
                            spec["seconds"] = 3;
                        console.warn(" token magic effects removing in ", spec);
                        //@ts-ignore
                        game.Gametime.doIn(spec, DynamicEffects.requestGMAction, DynamicEffects.GMAction.actions.deleteTokenMagic, { tokenId: token.id, effectId: fields[0] });
                    }
                    else {
                        const units = { seconds: 1, minutes: 60, hours: 3600, rounds: 6, turns: 3 };
                        const realDuration = (units[duration.units] || 1) * (duration.value || 3);
                        setTimeout(() => {
                            requestGMAction(GMAction.actions.deleteTokenMagic, { tokenId: token.id, effectId: fields[0] });
                        }, realDuration * 1000);
                    }
                }
                return;
...
let fixupComputedFields = (actor) => {
    ...
    if (game.system.id === "sw5e") {
        ...
        let flags = actor.data.flags.sw5e;
...
function expandSpecial(effect) {
    ...
    switch (effect.modSpecKey) {
        ...
        case "data.traits.di.all":
            return [new EffectModifier("data.traits.di.value", "=", Object.keys(CONFIG.SW5E.damageResistanceTypes))];
        case "data.traits.dr.all":
            return [new EffectModifier("data.traits.dr.value", "=", Object.keys(CONFIG.SW5E.damageResistanceTypes))];
        case "data.traits.dv.all":
            return [new EffectModifier("data.traits.dv.value", "=", Object.keys(CONFIG.SW5E.damageResistanceTypes))];
        default:
            return [effect];
    }
}
